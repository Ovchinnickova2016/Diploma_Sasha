...
/*     createTextNode(text,attributes){
    ...
    on(){
        ...
        if((this.id=="comp4")||(this.id=='ip4'))
        {
            document.getElementById('ip').value = '192.168.0.2';
        }
        if(this.id=='task')
        {
        ...
        if(this.id=='checkid')
        { var che=0; var l =0; var k2 = document.getElementById('rights').value;
            var k = parseInt(document.getElementById('rights').value);
        var count_num = 0;
        for (l=0; l<k2.length; l++)
        {
            if(isNumeric(k2[l])==false){
                count_num++; che++;
            }
        }
        if (count_num > 0){
            alert('В значении прав не может быть букв.');
        }
        var count = 0;
        for (l=0; l<k2.length; l++)
        {
        ...
        {
            alert("Некорректное значение для прав.");
        }
        
            if (k2.length == 0){
             alert("Пустые поля ввода."); che++;  
            
            }
        if (k2.length !=0 &&  k2.length!=3){alert("В значении прав должно быть всего 3 символа.");}
       if (che == 0 && k2.length == 3)
       {
           function checkAnswer2(){
                fetch("http://localhost:5000/labs1/check/", {
                    ...
                    body: JSON.stringify(
                        {
                                "num":   document.getElementById('rights').value
                        })
                  })
            }
            checkAnswer2();
...
class Rect2 extends Start{
    ...
    createElementSimple(name,attributes){
        ...
        if(name==='button')
            ...
            if  (name == "text"){
                this.element.innerHTML = "Права доступа к файлу:"
            }
